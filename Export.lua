-- DCS World Export Script - Fixed file operations
    ...
    local success, error_msg = pcall(function()
        ...
        if dataFile and now and nextUpdateTime and now > nextUpdateTime then
            ...
            if selfData then
                    ...
                    for stationId, station in pairs(payloadInfo.Stations) do
                        if station.weapon then
                            ...
                            table.insert(data.weapons, {
                                station = stationId,
                                name = station.weapon.displayName or "Unknown Weapon",
                ...
                -- Close and reopen file to avoid seek issues
                dataFile:close()
                dataFile = io.open(lfs.writedir().."Temp/dcs_overlay_data.json", "w")
                ...
                if dataFile then
                    ...
                    log.write('DCS-OVERLAY', log.ERROR, 'Failed to reopen file for writing')
                end
            else
...
-- Test at startup
log.write('DCS-OVERLAY', log.INFO, 'File operations fixed - no seek required')